...
#    path1 = './images/comparison/KF3' + r'/per_{}'.format(per) + r'/sam_' + str(j)
#    os.makedirs(path1)
...
def train(iterations, batch_size, sample_interval, mt_size, set_class, weight_per_class, get_samples, get_labels, num,
    ...
    for iteration in range(iterations):
        ...
        sup_samples, sup_labels, idx3 = dataset.batch_labeled(batch_size, get_samples, get_labels)
        ...
        #                   outputs=discriminator_net.get_layer('dense_5').output)
        ...
        #     plot_embedding_gen(result, label, 3, '{}% labels, epoch{}'.format(per, iteration + 1),
        ...
        #                    sup1="Generated samples  of model 3 in epoch{}".format(iteration + 1))
        #         plt.savefig('./images/comparison/KF3' + '/per_{}'.format(per) + '/sam_{}'.format(
        ...
        if (iteration >= 1) & (fscore_val > best_sc):
            ...
            y_te_3 = np.argmax(discriminator_supervised.predict(x_test), axis=1)
            test_score_3 = f1_score(y_test, y_te_3, average='macro')
            best_para = dis_wb * 1
            best_gen_para = gen_wb
            best_sc = fscore_val * 1
            best_test = test_score_3 * 1
        ...
        if iteration + 1 == iterations:
            ...
            for i in range(batch_size):
                ...
                plt_curves(sup_sam, figsize=(11, 5), dpi=420, sup1="The real samples of model 3")
                plt.savefig('./images/comparison/KF3' + '/per_{}'.format(per) + '/real_{}.jpg'.format(i + 1))
            #
            # plt.figure()
...
print('The third type of SK-Fold will begin:')
...
dis_wb3, fscore3, step3, gen_wb3 = train(iterations, batch_size, sample_interval, mt_size, set_class,
                                         weight_per_class, SAM_tr, LAB_tr, 3, x_val, y_val)
...
fscore_train_3 = np.mean(f1_score(y1, y1_pred, average=None))
