...
def train(iterations, batch_size, sample_interval, mt_size, set_class, weight_per_class, get_samples, get_labels, num,
    ...
    for iteration in range(iterations):
        ...
        sup_samples, sup_labels, idx2 = dataset.batch_labeled(batch_size, get_samples, get_labels)
        ...
        if (iteration >= 1) & (fscore_val > best_sc):
            ...
            y_te_2 = np.argmax(discriminator_supervised.predict(x_test), axis=1)
            test_score_2 = f1_score(y_test, y_te_2, average='macro')
            best_para = dis_wb * 1
            best_gen_para = gen_wb
            best_sc = fscore_val * 1
            best_test = test_score_2 * 1
        ...
        if iteration + 1 == iterations:
            ...
            for i in range(batch_size):
                ...
                plt_curves(sup_sam, figsize=(11, 5), dpi=420, sup1="The real samples of model 2")
                plt.savefig('./images/comparison/KF2' + '/per_{}'.format(per) + '/real_{}.jpg'.format(i + 1))
...
print('The second type of SK-Fold will begin:')
...
dis_wb2, fscore2, step2, gen_wb2 = train(iterations, batch_size, sample_interval, mt_size, set_class,
                                         weight_per_class, SAM_tr, LAB_tr, 2, x_val, y_val)
...
fscore_train_2 = np.mean(f1_score(y1, y1_pred, average=None))
